...
def build_chexpert_dual_attention(latent_dim, name):
...
class CheXpert001(base.Handler):
    ...
    def build(self):
        ...
        self.model = build_chexpert_dual_attention(latent_dim=128, name=None)
    ...
    def prepare(self):
        ...
        for buffer_index in self.inputs_buffer:
            ...
            for k in range(study_size):
                ...
                if not common.is_gray(self.intermediate_buffer[buffer_index]['image'][k]):
                    
                    self.intermediate_buffer[buffer_index]['image'][k]  = cv.cvtColor(self.intermediate_buffer[buffer_index]['image'][k], cv.COLOR_BGR2GRAY)    

                self.intermediate_buffer[buffer_index]['image'][k] = np.expand_dims(self.intermediate_buffer[buffer_index]['image'][k], axis=-1)
